...
(function (Sandbox) {
    ...
    function testDetect3() {
        ...
        maybeStream.then(function (stream) {
            ...
            for (var i = splitsize; i < stdscores.length; i += splitsize) {
                ...
                var _d = Statictics.findMax(stdpart), max_score = _d[0], offset = _d[1];
    ...
    function testDetect2() {
        ...
        maybeStream.then(function (stream) {
            ...
            for (var i = 0; i < clusterIds.length; i++) {
                var _d = Statictics.findMax(clusterIds[i].map(function (id) { return stdscores[id]; })), stdscore = _d[0], _id = _d[1];
    ...
    function testComplementaryCode(n) {
        if (n === void 0) { n = 0; }
        var _c = Signal.createComplementaryCode(n), a = _c[0], b = _c[1];
        console.log(0, a, b);
    ...
    function testScriptProcessor() {
        ...
        navigator.getUserMedia({ video: false, audio: true }, function (stream) {
            ...
            function handler(ev) {
                ...
                var _c = Signal.fft(buf, actx.sampleRate), real = _c[0], imag = _c[1], spectrum = _c[2];
                for (var i = 0; i < spectrum.length; i++) {
                    spectrum[i] = spectrum[i] * 20000;
                }
