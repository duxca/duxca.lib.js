...
var DFT = (function (_super) {
    ...
    function DFT(bufferSize, sampleRate) {
        var _this = _super.call(this, bufferSize, sampleRate) || this;
        var N = bufferSize / 2 * bufferSize;
        var TWO_PI = 2 * Math.PI;
        _this.sinTable = new Float32Array(N);
        _this.cosTable = new Float32Array(N);
        for (var i = 0; i < N; i++) {
            _this.sinTable[i] = Math.sin(i * TWO_PI / bufferSize);
            _this.cosTable[i] = Math.cos(i * TWO_PI / bufferSize);
        }
        return _this;
    }
    /**
...
var FFT = (function (_super) {
    ...
    function FFT(bufferSize, sampleRate) {
        var _this = _super.call(this, bufferSize, sampleRate) || this;
        _this.reverseTable = new Uint32Array(bufferSize);
        var limit = 1;
        var bit = bufferSize >> 1;
        var i;
        while (limit < bufferSize) {
            for (i = 0; i < limit; i++) {
                _this.reverseTable[i + limit] = _this.reverseTable[i] + bit;
            }
            limit = limit << 1;
            bit = bit >> 1;
        }
        _this.sinTable = new Float32Array(bufferSize);
        _this.cosTable = new Float32Array(bufferSize);
        for (i = 0; i < bufferSize; i++) {
            _this.sinTable[i] = Math.sin(-Math.PI / i);
            _this.cosTable[i] = Math.cos(-Math.PI / i);
        }
        return _this;
    }
    /**
...
var RFFT = (function (_super) {
    ...
    function RFFT(bufferSize, sampleRate) {
        var _this = _super.call(this, bufferSize, sampleRate) || this;
        _this.trans = new Float32Array(bufferSize);
        _this.reverseTable = new Uint32Array(bufferSize);
        _this.generateReverseTable();
        return _this;
    }
