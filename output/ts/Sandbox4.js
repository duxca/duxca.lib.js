    ...
    function testDetect5(rootNodeId) {
            ...
            chord.on("distribute", function (token, cb) {
                ...
                Object.keys(data).forEach(function (id1) {
                    Object.keys(data).forEach(function (id2) {
                        ...
                        console.log("__RES__", id1 + "-" + id2, "phaseShift", tmp, "ave", Statictics.average(results[id1 + "-" + id2]), "mode", Statictics.mode(results[id1 + "-" + id2]), "med", Statictics.median(results[id1 + "-" + id2]), "stdev", Statictics.stdev(results[id1 + "-" + id2]));
                    });
                });
                cb(token);
            });
        ...
        function calc(myId, pulse, pulseStartTime, pulseStopTime) {
            ...
            Object.keys(pulseStartTime).forEach(function (id) {
                ...
                var _a = Statictics.findMax(section), max_score = _a[0], max_offset = _a[1];
