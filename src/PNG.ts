...
export function uInt8ToBitArray(uint8: number): number[] {// 170 -> [1,0,1,0,1,0,1,0]
  return (uint8+256).toString(2).split("").slice(1).map(Number);
}

export function uInt8ArrayToBits(arr: Uint8Array): number[] {// new Uint8Array([170]) -> [1,0,1,0,1,0,1,0]
  var result:number[] = [];
...
export function bitsToNum(bits:number[]): number{// [1,0,1,0,1,0,1,0] -> 170
  //return bits.slice().reverse().reduce(function(sum,n,i){return sum+Math.pow(2,i)*n},0);
  return parseInt(bits.join(""), 2);
}

export function readBits(buffer: Uint8Array, bitOffset: number, bitLength:number ): number[]{
